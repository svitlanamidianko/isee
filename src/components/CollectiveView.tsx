...
const CollectiveView: React.FC = () => {
  ...
  const bind = useDrag(({ args: [index], active, movement: [mx], direction: [xDir], velocity }) => {
    ...
    if (!active && trigger) {
      ...
      if (rectanglesRef.current) {
        const newEntries = cards[(currentCardIndex + 1) % cards.length].text ? 
          [{ entry_text: cards[(currentCardIndex + 1) % cards.length].text }] : [];
  ...
  useEffect(() => {
    ...
    const setupMatterJs = () => {
      ...
      const entries = cards[currentCardIndex].text ? [{ entry_text: cards[currentCardIndex].text }] : [];
